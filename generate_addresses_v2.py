...
def collect_params():
    params = {}
    params["R"] = input("Retailer code (leave empty generate GOLD addresses) :".format(default_params["retailer_value"]))
    if params["R"]:
        # params["R"] = default_params["retailer_value"]
    ...
    params["S"] = input("Level (leave empty to set default value `{}`) format=`01-9`:".format(default_params["shelf_range"]))
    if not params["S"]:  params["S"] = default_params["shelf_range"]
    params["S"] = Range(*map(int,re.findall("(\d)-(\d)", params["S"])[0]))
    print("Level range = from {} to {}".format(params["S"].min, params["S"].max))

    params["P"] = input("Stack (leave empty to set default value `{}`) format=`A-J`:".format(default_params["position_range"]))
    if not params["P"]:  params["P"] = default_params["position_range"]
    params["P"] = Range(*map(lambda x:x.upper(),re.findall("(\w)-(\w)", params["P"])[0]))
    print("Stack range = from {} to {}".format(params["P"].min, params["P"].max))
...
def main():
    ...
    if R:
        ...
        filename = "gold_addresses_zone_" + Z + ".csv"
            ...
            for B in range(params["B"].min, params["B"].max + 1):
                for S in range(params["S"].min, params["S"].max + 1):
                    for P in alphbt[alphbt.find(params["P"].min):alphbt.find(params["P"].max) + 1]:
                        R, D, A = map(lambda x:str(x).rjust(2,"0"),(R, D, A))
                        B = str(B).rjust(3, "0")
                        out = f"{R},{D},{Z},{A},{B},{S},{P}"
